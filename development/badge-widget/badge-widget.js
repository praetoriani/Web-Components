...
Make sure that the 'bage-widget.css' is in the same directory as the 'badge-widget.js'.
The web component can then be used like this:

<badge-widget iconname="github" iconcolor="#303030" shadow="true" glossy="true" linkurl="https://github.com/praetoriani">
    <span slot="badgename" class="YourCSSclass"><strong>Github.com</strong></span>
    <span slot="badgetext" class="YourCSSclass FontBlack">/praetoriani</span>
</badge-widget>

iconname    (required)
Can have following options: 'github' , 'html' , 'css' , 'jscript' , 'chrome' , 'edge'
If you do not want to use an icon you simply pass an empty value to the attribute (iconname="")

iconcolor   (required)
Sets the color of the icon. Must be a HEX-Value like '#000000'
If you do not want to use an icon you simply pass an empty value to the attribute (iconcolor="")
...
linkurl     (optional)
Must be an url like 'https://www.github.com/praetoriani'. If you use this attribute, the badge-widget
will have an onclick-event and will show a pointer cursor (if the cursor hovers over the badge-widget)

<span slot="badgename" class="YourCSSclass"...</span>
<span slot="badgetext" class="YourCSSclass"...</span>
With these two slots you can write some text on the badge-widget. You do not have to use both slots.
But you have to use at least one of these slots. Do not change 'badgename' and 'badgetext'. They are
mandatory identifiers for the javascript code. Without these identifiers, your css code cannot be
attached to the web component.

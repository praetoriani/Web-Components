...
class BadgeWidget extends HTMLElement {
    ...
    async InjectCSS() {
        const res = await fetch('badge-widget.css');      // Fetch the CSS file
    ...
    RenderComponent() {
        ...
        this.shadowRoot.innerHTML = `
            <div id="SVGBatchBox" class="SVGBatchBox">
                <table class="BatchBoxTable" cellspacing="0" cellpadding="0">
                    <tr>
                        <td class="TabCellIcon">
                            <svg id="SVGiconHTML" class="SVGiconClass" xmlns="http://www.w3.org/2000/svg" viewBox="${this.IconView}">
                                <!--
                        ...
                        <td class="TabCellText gFontw700">${this.varText}</td>
                        <td class="TabCellInfo gFontw400">${this.varInfo}</td>
                    </tr>
                </table>
            </div>
        `;
        this.UpdateComponent();     // ALWAYS call this, AFTER the shaddow DOM was created
    }
    ...
    */
    UpdateComponent() {
        ...
        this.iscolor =  regex.test(this.varColor);
        if( this.iscolor == true ) {
            this.shadowRoot.getElementById('SVGiconHTML').style.fill = this.varColor;
            this.shadowRoot.getElementById('SVGiconHTML').style.opacity = 0.90;
        } else {
            this.shadowRoot.getElementById('SVGiconHTML').style.fill = "#F3F3F3";
            this.shadowRoot.getElementById('SVGiconHTML').style.opacity = 0.80;
        }
        // Update the shadow depending on the value of the component's attribute
        if( this.Shadow == "true" ) {
            this.shadowRoot.getElementById('SVGBatchBox').style.boxShadow = "4px 4px 8px rgba(0, 0, 0, 0.5)";
        } else {
            this.shadowRoot.getElementById('SVGBatchBox').style.boxShadow = "0px 0px 0px rgba(0, 0, 0, 0.0)";
        }
        // Update the glossy effect depending on the value of the component's attribute
        if( this.Glossy == "true" ) {
            this.shadowRoot.getElementById('SVGBatchBox').style.backgroundImage = "linear-gradient(to bottom, #ffffff99, #dfdfdf66, #bebebe33, transparent)";
        } else {
            this.shadowRoot.getElementById('SVGBatchBox').style.backgroundImage = "none";
        }
    }

    CatchAttributes() {
        // Catch the attribute values from the component
        this.varIcon  = this.getAttribute('icon');
        this.varText  = this.getAttribute('text');
        this.varInfo  = this.getAttribute('info');
        this.varColor = this.getAttribute('color');
        this.Shadow   = this.getAttribute('shadow');
        this.Glossy   = this.getAttribute('glossy');
    }

    LoadSVG() {
        // Load the SVG from the global object library, depending on the attribute value of our component
        switch( String(this.varIcon.toLowerCase()) ) {
            case 'github':
...
// Define the new element
customElements.define('badge-widget', BadgeWidget);